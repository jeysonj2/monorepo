...
import type { Data as PopupData, RemoveEventType, SubmitEventType } from '@interzero-pmp/flow-option-popup';
...
export class FlowContent extends LitElement {
  ...
  private handleOptionClick(event:CustomEvent<OptionEvent>) {
    ...
    switch (event.detail.variant)
    {
      ...
      case "save": {
        this.dispatchEvent(new CustomEvent<CompleteEvent>("complete", {
          detail: {
            type: "save",
            data: this.data
          } 
      ...
      default: {
        console.log("theres no other variants..");
        break;
      }
    }
