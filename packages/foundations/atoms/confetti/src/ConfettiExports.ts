...
// import PopSoundSource from './sound/pop.mp3';
...
export class Confetti extends LitElement {
  ...
  private getPS(placement:Placement) {
    ...
    return new ParticleSystem(x, y, angle, this.height, ["yellow", "cornflowerblue", "red", "green", "orange"]);
  }
  ...
  public async shoot(randomDelayMax = 800, amount = 20, spread = Math.PI/3) {
    ...
    for (let ps of this.particlesystems) 
    {
      ...
      ps.burst(~~(amount + Math.random()*amount), spread);
      await wait(~~(Math.random() * randomDelayMax + randomDelayMax/2))
    }
  ...
  render() {
    return html`
      <canvas>You need JavasScript enabled to run HTML5 canvas! Consider switching to Chrome..</canvas>
    `;
  }
}
