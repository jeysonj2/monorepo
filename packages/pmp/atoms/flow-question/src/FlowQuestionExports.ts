...
export class FlowQuestion extends LitElement {
  ...
  private handleInput(name:NameType) {
    return (event:CustomEvent<InputEventChangeInfo>) => {
      this.dispatchEvent(new CustomEvent<ChangeEvent>("change", {detail: {value:event.detail.value, name}}));
    }
  }
    ...
    if (this.editMode)
    {
      return html`
        <iz-field label="Question">
          <iz-input @suspended-input-change=${this.handleInput("question")} .value=${this.question} placeholder="type a question"></iz-input>
        </iz-field>
        <iz-field label="Description">
          <iz-input @suspended-input-change=${this.handleInput("description")} .value=${this.description} placeholder="type a description"></iz-input>
        </iz-field>
      `;
    }
