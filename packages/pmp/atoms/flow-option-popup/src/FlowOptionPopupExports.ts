...
// event types 
export type SubmitEventType = { id?: string; parent?: string; value: string; priceAffection: string; priceAffectionType: PriceAffectionType; };
...
export class FlowOptionPopup extends LitElement {
  ...
  private handleRemove() {
    this.dispatchEvent(new CustomEvent<RemoveEventType>("remove", { detail: { id: this.data?.id as string, parent: this.data?.parent }}))
  }
  private handleSubmit = (event:Event) => {
    ...
    const priceAffectionType = formdata.get("priceAffectionType") as PriceAffectionType;

    this.dispatchEvent(new CustomEvent<SubmitEventType>("submit", { detail: { id: this.data?.id, parent: this.data?.parent, value, priceAffection, priceAffectionType }}));
  }
    ...
    return html`
      ...
      >
        <iz-field label='Value'>
          <iz-input required name="value" placeholder="Type a value" value=${this.data?.value||''}></iz-input>
        </iz-field>
        <iz-field label='Price Affection'>
          <iz-input required type="number" name="priceAffection" placeholder="Type a price-affection" value=${this.data?.priceAffection||''}></iz-input>
        </iz-field>
        <iz-field label='Price Affection Type'>
          <iz-dropdown value=${this.data?.priceAffectionType||""} name="priceAffectionType">
        ...
        ${(this.data === undefined || this.data === null || this.data.id === undefined) ? html`
