...
import type {
  Variant as OptionVariant,
  ClickEvent as OptionClickEvent,
} from '@interzero-pmp/flow-option';
...
export class FlowView extends LitElement {
  ...
  private handleQuestionChange(event: CustomEvent<QuestionChangeEvent>) {
    this.dispatchEvent(
      new CustomEvent<ChangeEvent>('question-change', {
        detail: { ...event.detail, id: this.data.id },
      })
    );
  }
  ...
  private handleOptionClick(variant: OptionVariant) {
    return (event: CustomEvent<OptionClickEvent>) => {
      this.dispatchEvent(
        new CustomEvent<OptionEvent>('option-click', {
          detail: { id: this.data?.id, optionId: event.detail?.id, variant },
        })
      );
    };
  }
  ...
  render() {
    return html`
      ...
      <div class="grid">
        ...
        ${this.variant === 'edit' && this.data?.options.length === 0
          ? html`
              <iz-pmp-flow-option
                variant="save"
